...
export const getEthProvider = () => {
  return (
    ...
    ethers.getDefaultProvider(hre.network.config.ethNetwork)
  );
};
...
export const getGovernance = () => {
  if (!process.env.GOVERNANCE_ADDRESS)
    throw "⛔️ Governance address wasn't found in .env file!";
  return process.env.GOVERNANCE_ADDRESS;
};

export const getWhitelistAdmin = () => {
  if (!process.env.WHITELIST_ADMIN_ADDRESS)
    throw "⛔️ Whitelist admin address wasn't found in .env file!";
...
export const deployContract = async (
  ...
  if (!options?.skipChecks) {
    ...
    log(
      `Estimated total deployment cost: ${ethers.formatEther(deploymentFee)} ETH`,
    );
