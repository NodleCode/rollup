...
contract ClickBountyTest is Test {
    ...
    function setUp() public {
        ...
        clickBounty = new ClickBounty(oracle, address(token), address(contentSign), admin);
    ...
    function testInitialState() public view {
        ...
        assertEq(fee, 100, "Initial entry fee must be 100");
    ...
    function testAwardBountyAmountMatchingEntryFee() public {
        ...
        assertEq(clickBounty.bounties(tokenId), fee, "Bounty must be given to token owner");
    }
    ...
    function testAwardBountyAmountLessThanFee() public {
        ...
        token.approve(user1, fee);
        token.transfer(user1, fee);
        ...
        assertEq(token.balanceOf(user1), fee - 1, "User1 should have been refunded partially");
    }

    function testAwardBountyAmountLessThanFeeButNoUserApprovalForFee() public {
        ...
        vm.prank(oracle);
        vm.expectRevert(
            abi.encodeWithSelector(IERC20Errors.ERC20InsufficientAllowance.selector, address(clickBounty), 0, 1)
        );
        clickBounty.awardBounty(tokenId, fee - 1);
    }
    ...
    function testAwardBountyAmountGreaterThanFee() public {
        ...
        token.approve(address(clickBounty), 1);
        token.transfer(address(clickBounty), 1);
    ...
    function testAwardBountyFailsIfTokenDoesNotExist() public {
        ...
        clickBounty.awardBounty(randomNonExistingTokenId, fee);
    }
    ...
    function testLeaderboardReplacement() public {
        ...
        ClickBounty _clickBounty = new ClickBounty(oracle, address(token), address(contentSign), admin);
        ...
        token.approve(address(_clickBounty), 10000);
