...
// Check if social handle is available
app.get(
  "/social/availability",
  [
    query("platform")
      .isIn(['com.twitter'])
      .withMessage("Platform must be com.twitter"),
    query("handle")
      .isString()
      .isLength({ min: 1, max: 50 })
      .withMessage("Handle must be 1-50 characters"),
  ],
  async (req: Request, res: Response) => {
    try {
      const result = validationResult(req)
      ...
      const { platform, handle } = matchedData(req)
      const claimStatus = await SocialValidationService.isHandleClaimed(platform, handle)

      res.status(200).json({
        available: !claimStatus.claimed,
        claimedBy: claimStatus.ensName,
...
// Reserve social handle for ENS name
app.post(
  "/social/reserve",
  [
    body("ensName")
      .isString()
      .withMessage("ENS name is required"),
    body("platform")
      .isIn(['com.twitter'])
      .withMessage("Platform must be com.twitter"),
    body("handle")
  ...
  async (req: Request, res: Response) => {
    try {
      // Firebase auth check (optional - you might want to remove this for app-side handling)
      ...
      const { ensName, platform, handle, owner } = matchedData(req)

      // Reserve the handle
      const reservation = await SocialValidationService.reserveHandle(
        ensName, platform, handle, owner
      )

      res.status(200).json({
        verificationCode: reservation.verificationCode,
        expiresAt: reservation.expiresAt,
        message: "Handle reserved successfully. You can now set the text record."
      })
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error)
